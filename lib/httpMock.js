...
console.log("  Mean Value for Waiting Time (Gaussian curve) i.e. medium value: " + meanValue);
console.log("  Standard Deviation value for Waiting Time : " + deviationValue);

var ruleManager = rule.create_ruleManager(file);

var randomWaitingTime = function(startTime, rule) {
  var meanValueFinal = rule.responseTimeInMilli ? rule.responseTimeInMilli : meanValue,
      deviationValueFinal = rule.deviationValue ? rule.deviationValue : deviationValue,
      computationTime = new Date().getMilliseconds() - startTime.getMilliseconds();

  var result = random.get(meanValueFinal, deviationValueFinal);
  console.log("  Random waiting time " + result + "ms / Computation time : " + computationTime + "ms");
  return result;
...
http.createServer(function(request, response) {
	var content = "",
      startTime = new Date();
	console.log("******* New Request (" + startTime.toISOString() + ") *******");
    ...
    request.on('end', function () {
      ...
      setTimeout(function() {
        ...
        }, randomWaitingTime(startTime, rule));
		});
	})
}).listen(port, address);
